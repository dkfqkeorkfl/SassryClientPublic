    ...
    namespace Component {
        ...
        public class Account {
            ...
            public Account (Context ctx) {
                    ...
                    context = ctx;
                    context.paylod_handler += PayloadHandle;
                }
			...
			void PayloadHandle (JObject token) {
				if (token ["/Account/Login"] != null) {
					context.login_result = token ["/Account/Login"].ToObject<AccountLoginResult> ();
				} else if (token ["/Account/AccessOpen"] != null) {
					var equip = token ["/Account/AccessOpen"].ToObject<AccountAccessEquipResult> ();
                    context.login_result.equips = context.login_result.equips
                        .Where (e => e.id == equip.id)
                        .DefaultIfEmpty (null)
                        .Select (e => e != null ? e : equip)
                        .ToArray ();
				} else if (token["/Account/AccessDel"] != null) {
					var targets = token ["/Account/AccessDel"].ToObject<ulong[]>();
                    context.login_result.equips = context.login_result.equips
